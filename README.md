...
### Structure
This project is split up into three directorys. Each directory contains its own `README.md`-file to better explain the content and the function of the directory. As a quick overview, the [`/docker`](/docker) directory contains everything relating to the translator app itself. The [`/terraform`](/terraform) directory contains the necessary directives to deploy a VM on a Microsoft Azure server. Lastly, the [`/ansible`](/ansible) directory contains the directives to install docker and deploy the container onto the VM.
