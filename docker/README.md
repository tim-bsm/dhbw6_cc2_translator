...
Any API keys, passwords etc. should be provided via environemnt variables. For the app to work, the following variables have to be provided in the [`compose.yaml`](/docker/compose.yml) (or even better an `.env`-file inside the [`docker`](/docker) directory).
...
The folder contains a normal [`compose.yml`](/docker/compose.yml) file to pull the latest build of the translator app inside the [`src`](/docker/src)-folder. The [`dev_compose.yml`](/docker/dev_compose.yml) on the other hand is used for developing the translator app inside of the [`src`](/docker/src)-folder. 
